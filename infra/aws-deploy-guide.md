...
Tên bucket S3 dùng chung namespace cho toàn bộ khách hàng AWS, không riêng tài khoản này.
`shopflow-terraform-state` trần đã có người lấy mất, nên hậu tố là bắt buộc chứ không phải phòng
xa. Lấy account id làm hậu tố thì tên tự dựng được ở mọi chỗ cần dùng, khỏi phải nhớ.
...
`plan` bắt được tham chiếu gãy, sai kiểu, vi phạm luật `validation` của biến (như luật cấm thẻ
`latest` ở `api_image`), và thiếu quyền đọc. Nó **không** bắt được những lỗi chỉ lộ ra khi tạo thật:
một cỡ máy không có ở vùng đã chọn, hết hạn mức tài khoản, hay tên tài nguyên đã có người lấy. Đó
là lý do đặt tạm ba biến ở trên vẫn `plan` được — Terraform không kiểm tra chúng có thật hay không.

Giá trị lớn nhất của bước này là danh sách tài nguyên sắp tạo. Đọc kỹ output. Không mất đồng nào
để đọc.
